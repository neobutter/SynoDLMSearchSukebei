...
class SynoDLMSearchSukebei {
	...
	$response = preg_replace_callback("/([0-9]+)(?=<\/nyaa:seeders>).+([0-9]+)(?=<\/nyaa:leechers>).+([0-9a-zA-Z]+]]>)/sU", create_function('$matches', 'return str_replace($matches[3], sprintf("Ratio : %s seeds %s leechers | %s", $matches[1], $matches[2], $matches[3]), $matches[0]);'), $response);
